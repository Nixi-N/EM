...
// 用于生成 TG 播报消息的核心工具函数 (还原：二改大屏图文并茂版)
async function sendTgStats(env, chatId) {
    try {
        ...
        // ✨ 时间问候语生成
        // ==========================================
        const localNow = new Date(new Date().getTime() + 8 * 3600000);
        ...
        try {
            ...
            if (ipData.status === 'success') {
                egressIp = ipData.query;
                egressLoc = `${ipData.country} ${ipData.city}`; 
            } else {
                const altIpRes = await fetch('https://api.ipify.org?format=json');
                const altIpData = await altIpRes.json();
        ...
        const reqCount = todayReq ? todayReq.count : 0;
        ...
        const ipCount = todayIPs ? todayIPs.count : 0;

        let peakHourText = '暂无记录';
        try {
            const { results: tsLogs } = await env.DB.prepare("SELECT timestamp FROM visitor_logs WHERE date(timestamp, '+8 hours') = date('now', '+8 hours')").all();
            if (tsLogs && tsLogs.length > 0) {
                ...
                tsLogs.forEach(row => {
                    ...
                    if (typeof row.timestamp === 'number' || (typeof row.timestamp === 'string' && /^\d+$/.test(row.timestamp))) {
                        ...
                        let d = new Date(safeTs);
                        if (!isNaN(d.getTime())) {
                            hStr = new Date(d.getTime() + 8 * 3600000).getUTCHours();
                        }
                    }
                    
                    if (hStr !== undefined && !isNaN(hStr)) {
                ...
                let maxH = -1, maxC = 0;
                for (let h = 0; h < 24; h++) {
                    if (hourMap[h] > maxC) { maxC = hourMap[h]; maxH = h; }
                }

                if (maxH >= 0) {
                    let nextH = maxH === 23 ? 0 : maxH + 1;
                    peakHourText = `${String(maxH).padStart(2, '0')}:00 - ${String(nextH).padStart(2, '0')}:00 (${maxC}次)`;
                }
            }
        } catch(e) {}
        ...
        const totalDbCount = totalDbQuery ? totalDbQuery.c : 0;
        const DB_LIMIT = 50000; 
        const dbUsagePct = ((totalDbCount / DB_LIMIT) * 100).toFixed(1);
        ...
        let clientText = '\n • <i>今日暂无设备数据</i>';
        try {
            const { results: uaResults } = await env.DB.prepare("SELECT ua, COUNT(*) as c FROM visitor_logs WHERE date(timestamp, '+8 hours') = date('now', '+8 hours') AND ua IS NOT NULL AND ua != '' GROUP BY ua").all();
            let stats = { infuse: 0, hills: 0, emby: 0, jellyfin: 0, vidhub: 0, fileball: 0, senplayer: 0, tv: 0, web: 0, others: 0 };
            let totalUaCount = 0;
            let sampleOtherUa = ''; 
            let maxOtherC = 0;

            if (uaResults && uaResults.length > 0) {
                uaResults.forEach(r => {
                    let ua = (r.ua || '').toLowerCase();
                    let c = r.c; 
                    totalUaCount += c;

                    if (ua.includes('infuse')) stats.infuse += c;
                    ...
                    else if (ua.includes('okhttp') || ua.includes('exoplayer') || ua.includes('dalvik')) stats.tv += c;
                    else if (/(mozilla|chrome|safari|edge|applewebkit)/i.test(ua)) stats.web += c;
                    else {
                        stats.others += c;
                        if (c > maxOtherC) { maxOtherC = c; sampleOtherUa = r.ua; }
                    }
                });
                ...
                if (totalUaCount > 0) {
                    clientText = '';
                    if (stats.hills > 0) clientText += `\n • 🍿 Hills：<code>${getPct(stats.hills, totalUaCount)} (${stats.hills}次)</code>`;
                    if (stats.infuse > 0) clientText += `\n • 🍎 Infuse：<code>${getPct(stats.infuse, totalUaCount)} (${stats.infuse}次)</code>`;
                    if (stats.senplayer > 0) clientText += `\n • 🍏 SenPlayer：<code>${getPct(stats.senplayer, totalUaCount)} (${stats.senplayer}次)</code>`;
                    if (stats.vidhub > 0) clientText += `\n • 🎬 VidHub：${getPct(stats.vidhub, totalUaCount)} (${stats.vidhub}次)`;
                    if (stats.fileball > 0) clientText += `\n • 📁 Fileball：${getPct(stats.fileball, totalUaCount)} (${stats.fileball}次)`;
                    if (stats.emby > 0) clientText += `\n • 🎬 Emby 原生：${getPct(stats.emby, totalUaCount)} (${stats.emby}次)`;
                    if (stats.tv > 0) clientText += `\n • 📺 电视/盒子端：${getPct(stats.tv, totalUaCount)} (${stats.tv}次)`;
                    if (stats.web > 0) clientText += `\n • 🌐 浏览器端：${getPct(stats.web, totalUaCount)} (${stats.web}次)`;
                    if (stats.others > 0) {
                        let shortUa = sampleOtherUa.split(' ')[0].substring(0, 15);
                        clientText += `\n • 📦 其他 (${escapeHTML(shortUa)}...)：${getPct(stats.others, totalUaCount)} (${stats.others}次)`;
                    }
                }
            }
        } catch(e) {}
        ...
        // 3. CF 大盘流量获取
        // ==========================================
        let trafficToday = '获取失败', traffic7d = '获取失败';
        try {
        ...
        if (env.CF_API_TOKEN && env.CF_ZONE_ID && env.DB) {
            try {
                const { results: routes } = await env.DB.prepare("SELECT * FROM routes").all();
                if (routes && routes.length > 0) {
                    ...
                    await Promise.all(routes.map(async route => {
                        try {
                            const query = `query { viewer { zones(filter: {zoneTag: "${env.CF_ZONE_ID}"}) { httpRequestsAdaptiveGroups(limit: 10000, filter: {clientRequestPath_like: "/${route.prefix}%", datetime_geq: "${startUtc}", datetime_leq: "${endUtc}"}) { sum { edgeResponseBytes } } } } }`;
                            const res = await fetch("https://api.cloudflare.com/client/v4/graphql", {
                                method: "POST", headers: { "Authorization": "Bearer " + env.CF_API_TOKEN, "Content-Type": "application/json" }, body: JSON.stringify({ query })
                            });
                            ...
                            if (reqs > 0 || bytes > 0) {
                                nodeDataList.push({ name: route.remark || route.prefix, bytes: bytes, reqs: reqs, rawTarget: route.target });
                            }
                        } catch(e){}
                    }));
                    ...
                    for (let n of top3) {
                        n.pingMs = '超时';
                        if (n.rawTarget) {
                            const tUrl = n.rawTarget.split(',')[0].trim();
                            const pStart = Date.now();
                            try {
                                const c = new AbortController();
                                const id = setTimeout(() => c.abort(), 1500); 
                                await fetch(tUrl, { method: 'OPTIONS', signal: c.signal });
                                clearTimeout(id);
                    ...
                    nodeDetails = nodeDataList.slice(0, 10).map((node, index) => {
                        ...
                        let b = node.bytes;
                        let bytesStr = "0 B";
                        if (b >= 1099511627776) bytesStr = (b / 1099511627776).toFixed(2) + "TB";
                        else if (b >= 1073741824) bytesStr = (b / 1073741824).toFixed(2) + "GB";
                        else if (b >= 1048576) bytesStr = (b / 1048576).toFixed(2) + "MB";
                        else if (b >= 1024) bytesStr = (b / 1024).toFixed(2) + "KB";
                        else if (b > 0) bytesStr = b + "B";

                        let extraAlert = '';
                        if (node.pingMs) {
                            const isTimeout = node.pingMs === '超时';
                            extraAlert += ` | ${isTimeout ? '🔴 掉线' : `♢${node.pingMs}`}`;
                        }
                        if (b > 536870912000) extraAlert += ' ⚠️ 流量飙升'; 

                        return `${icon} ${escapeHTML(node.name)}: <code>${bytesStr} | ${node.reqs}次${extraAlert}</code>`;
                    });
                }
            } catch (e) {}
        }
...
━━━━━━━━━━━━━━━━━━━━
 ...
 ┋ 顶级节点：<code>${escapeHTML(topIpText)}</code>

  ════客户端画像 (协议/设备)════${clientText}

 ┏┅┅┅┅系统状态与资源监控┅┅┅┅┅
 ┇ DB占用率：<code>${dbUsagePct}%</code> 
 ┇ 今日流量：<code>${trafficToday}</code>
 ┇ 七日流量：<code>${traffic7d}</code>
 ┇ 服务器区：<code>${colo}</code>
 ┇ 落地节点：<code>${egressIp}</code> 
 ┇ 落地地点：<code>${egressLoc}</code>
 ┗┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅
════ 核心线路状态 (TOP 10) ════
${nodeDetails.length > 0 ? nodeDetails.slice(0, 10).join('\n') : ' 🧊 <i>系统处于待机，暂无流量产生</i>'}
━━━━━━━━━━━━━━━━━━━━
⏱️ 统计时间：<code>${dateStr}</code>`;
        ...
        let lastMsgId = lastMsgRecord ? lastMsgRecord.value : null;
        ...
        const replyMarkup = {
            inline_keyboard: [
                [
                    { text: "🌐 测速面板", url: `https://${env.CF_DOMAIN}` },
        ...
        try {
            ...
            TOP_IMAGE_URL = "https://www.bing.com" + bingData.images[0].url;
        } catch (e) {
            TOP_IMAGE_URL = "https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=1920"; 
        }

        // 使用原生图片 API (editMessageMedia 和 sendPhoto) 铺满屏幕
        if (lastMsgId) {
            const editRes = await fetch(`https://api.telegram.org/bot${env.TG_BOT_TOKEN}/editMessageMedia`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    chat_id: chatId,
        ...
        if (sendNewMessage) {
            const sendRes = await fetch(`https://api.telegram.org/bot${env.TG_BOT_TOKEN}/sendMessage`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    chat_id: chatId,
            ...
            // 为了兼容某些 Telegram API 老版本报错，这里做一个智能降级处理
            const sendJson = await sendRes.json();
            if (sendJson.error_code === 400 && sendJson.description.includes('must be sent as a photo')) {
                const retryRes = await fetch(`https://api.telegram.org/bot${env.TG_BOT_TOKEN}/sendPhoto`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        chat_id: chatId,
                ...
                if (retryJson.ok && retryJson.result.message_id) {
                    await env.DB.prepare("INSERT OR REPLACE INTO kv_store (key, value) VALUES (?, ?)")
                        .bind(MSG_ID_KEY, String(retryJson.result.message_id)).run();
                }
            } else if (sendJson.ok && sendJson.result.message_id) {
                await env.DB.prepare("INSERT OR REPLACE INTO kv_store (key, value) VALUES (?, ?)")
            ...
            try {
                ...
                if (body.callback_query && body.callback_query.data === 'refresh_stats') {
                    if (env.DB && env.TG_BOT_TOKEN) {
                        ...
                        // [关键修复 2] 强制同步当前点击的 message_id 到数据库，确保 100% 覆盖你点的那条消息
                        await env.DB.exec(`CREATE TABLE IF NOT EXISTS kv_store (key TEXT PRIMARY KEY, value TEXT)`);
                        await env.DB.prepare("INSERT OR REPLACE INTO kv_store (key, value) VALUES (?, ?)")
                            .bind(`tg_last_msg_id_${chatId}`, String(msgId)).run();

                        // [关键修复 3] 重新拉取数据并更新面板 (用 await 阻止 Worker 提早休眠)
                        await sendTgStats(env, chatId);
                    }
                }
